/*
 * Smart Cyber Clock v10.1 (ST7789)
 * ================================
 * Changes from v5/v6:
 *   - HA LED control simplified:
 *     - Removed light entity (was causing mode conflicts)
 *     - Added "Backlight Dimmer" number entity (0-255 slider)
 ...
 *
 * Menu: Monitor, Pomodoro, Alarm, Snake, Tetris, Backlight, Ayarlar
 */
...
const char* mqtt_client_id = "cyberclock";

// MQTT Topics
#define TOPIC_PREFIX       "cyberclock"
...
#define TOPIC_BL_SET       TOPIC_PREFIX "/backlight/set"
#define TOPIC_BL_STATE     TOPIC_PREFIX "/backlight/state"
...
#define TOPIC_WEATHER      TOPIC_PREFIX "/weather"
...
#define SDA_PIN 1
#define SCL_PIN 2
#define BL_PIN   4    // Backlight (was Backlight)
#define BUZZ_PIN 3
...
#define CYBER_GREY    0x9CF3    // Light grey for labels (close to white)
...
// ====== Backlight (declared early for saveSettings/loadSettings) ======
uint8_t backlightLevel = 200;  // 0-255 PWM
...
void loadSettings() {
  ...
  backlightLevel = prefs.getUChar("backlight", 200);
...
void vTone(int pin, int freq, int durMs) {
  ...
  // Stop previous tone briefly for note separation
  if (buzzAttached) {
    ledcWrite(pin, 0);
    ledcDetach(pin);
    delayMicroseconds(800);  // tiny gap between notes
  }
...
// ====== Modes ======
enum UIMode { MODE_MENU=0, MODE_CLOCK, MODE_POMODORO, MODE_ALARM, MODE_GAMES_MENU, MODE_GAME, MODE_TETRIS, MODE_BREAKOUT, MODE_FLAPPY, MODE_PONG, MODE_2048, MODE_BRIGHT, MODE_HOME, MODE_SETTINGS, MODE_SLEEP };
UIMode currentMode = MODE_CLOCK;
UIMode preSleepMode = MODE_CLOCK;  // Remember mode before sleep
int menuIndex = 0;
const int MENU_ITEMS = 8;  // Monitor, Pomodoro, Alarm, Oyunlar, Backlight, Ev, Ayarlar, Uyku
...
bool screenDimmed = false;               // Screen is dimmed (pre-sleep)
unsigned long dimStartMs = 0;            // When dimming started
const unsigned long DIM_TO_SLEEP_MS = 120000;  // 2 min from dim to full sleep
unsigned long lastActivityMs = 0;        // Last user interaction timestamp
unsigned long k0HoldStart = 0;           // K0 long press detection
bool k0Held = false;

// ====== Backlight ======
// (backlightLevel declared above for saveSettings/loadSettings)
...
const char* rtttlAlarms[] = {
  ...
  "Macarena:d=8,o=5,b=180:f,f,f,4f,f,f,f,f,f,f,f,a,c,p,f,f,f,4f,f,f,f,f,f,f,d,c",
};
...
// selectedAlarm declared earlier for saveSettings/loadSettings
...
int ensLastStatus = -1;  // ENS160 last measure status for on-screen debug

// ====== Weather from HA ======
// Format: {"temp":18.5,"hum":45,"cond":"Açık","wind":12.3}
float weatherTemp = 0;
int weatherHum = 0;
float weatherWind = 0;
char weatherCond[32] = "";
...
void drawGamesMenu();
// Tetris uses landscape mode (320x240) — grid left, info right
// Layout: Left = grid (10*11=110px), Right = score/next/info (200px)
#define TET_COLS 10
#define TET_ROWS 20
#define TET_BLOCK 11   // 10*11=110px wide, 20*11=220px tall
#define TET_X 10       // Left margin
#define TET_Y 10       // Top margin (20*11=220, fits 240)
...
void tetDrawScore() {
  int px = TET_X + TET_COLS*TET_BLOCK + 4;
  tft.fillRect(px, 38, 100, 30, CYBER_BG);
  tft.setTextSize(1); tft.setTextColor(CYBER_DARK);
  tft.setCursor(px, 38); tft.print("Skor");
  tft.setTextSize(2); tft.setTextColor(CYBER_GREEN, CYBER_BG);
  tft.setCursor(px, 50); tft.printf("%d", tetScore);
}

void tetDrawNext() {
  int px = TET_X + TET_COLS*TET_BLOCK + 4;
  int oy = 80;
  // Clear entire next piece area (prevent overlapping from previous piece)
  tft.fillRect(px, oy, 80, 70, CYBER_BG);
  tft.setTextSize(1); tft.setTextColor(CYBER_DARK);
  tft.setCursor(px, oy); tft.print("Siradaki:");
  for (int r = 0; r < 4; r++)
    for (int c = 0; c < 4; c++)
      if (tetGetBlock(tetNextPiece, 0, r, c))
        tft.fillRect(px + c*14, oy + 14 + r*14, 13, 13, tetColors[tetNextPiece]);
}
...
void initTetris() {
  ...
  // Stay in landscape mode (320x240)
  tft.setRotation(3);
  
  tft.fillScreen(CYBER_BG);
  
  // Left: game grid border (full height)
  tft.drawRect(TET_X-1, TET_Y, TET_COLS*TET_BLOCK+2, TET_ROWS*TET_BLOCK, ST77XX_WHITE);
  
  // Right panel (x=83 to 127, ~45px wide)
  int px = TET_X + TET_COLS*TET_BLOCK + 4;  // 85
  
  // Title
  tft.setTextSize(2); tft.setTextColor(CYBER_LIGHT, CYBER_BG);
  tft.setCursor(px, 10); tft.print("TETRIS");
  
  // Divider line
  tft.drawFastHLine(px, 32, 80, CYBER_DARK);
  
  // Controls help (bottom of right panel)
  tft.setTextColor(CYBER_DARK);
  tft.setCursor(px, 170); tft.print("Enc: L/R");
  tft.setCursor(px, 185); tft.print("Bas: Don");
  tft.setCursor(px, 200); tft.print("Uzun: Dusur");
  tft.setCursor(px, 215); tft.print("K0: cik");
...
void updateTetris(int enc, bool encP, bool back) {
  ...
  if (tetGameOver) {
    int px = TET_X + TET_COLS*TET_BLOCK + 4;
    tft.fillRect(px, 38, 190, 120, CYBER_BG);
    tft.setTextSize(3); tft.setTextColor(ST77XX_RED, CYBER_BG);
    tft.setCursor(px, 40); tft.print("GAME");
    tft.setCursor(px, 70); tft.print("OVER!");
    tft.setTextSize(2); tft.setTextColor(CYBER_GREEN, CYBER_BG);
    tft.setCursor(px, 110); tft.printf("Skor: %d", tetScore);
    tft.setTextSize(1); tft.setTextColor(CYBER_DARK);
    tft.setCursor(px, 140); tft.print("Bas: yeni oyun");
...
#define BRK_AREA_W 260   // Game area width
#define BRK_AREA_H 240   // Full height
#define BRK_AREA_X 0
#define BRK_PANEL_X 264  // Right panel start
#define BRK_COLS 13
#define BRK_ROWS 5
#define BRK_BRICK_W 20   // 13*20=260
#define BRK_BRICK_H 10
#define BRK_BRICK_Y 4    // Bricks start Y
#define BRK_PAD_W 52
#define BRK_PAD_H 6
#define BRK_PAD_Y 228    // Paddle near bottom
...
void brkDrawBall(uint16_t color) {
  tft.fillRect((int)brkBallX, (int)brkBallY, 5, 5, color);
}
...
void updateBreakout(int enc, bool encP, bool back) {
  ...
  if (enc != 0) {
    ...
    brkPadX = constrain(brkPadX + enc * 8, BRK_PAD_W/2, BRK_AREA_W - BRK_PAD_W/2);
    brkDrawPaddle(ST77XX_WHITE);
    if (!brkBallActive) {
  ...
  if (newY >= BRK_PAD_Y - 3 && newY <= BRK_PAD_Y && brkBallDY > 0) {
    ...
    if (newX + 5 >= padLeft && newX <= padRight) {
      brkBallDY = -brkBallDY;
      // Angle based on where ball hits paddle
      float hitPos = (newX - padLeft) / (float)BRK_PAD_W;  // 0..1
      brkBallDX = (hitPos - 0.5f) * 5.0f;  // -1.5 to +1.5
      if (brkBallDX > -0.8f && brkBallDX < 0.8f) brkBallDX = (brkBallDX < 0) ? -0.8f : 0.8f;
      newY = BRK_PAD_Y - 4;
...
#define FLP_AREA_W 260
#define FLP_AREA_H 240
#define FLP_PANEL_X 264
#define FLP_BIRD_X 50      // Bird fixed X position
#define FLP_BIRD_SIZE 12
#define FLP_PIPE_W 24
#define FLP_GAP 68         // Gap between top and bottom pipes (36*1.875≈68)
#define FLP_PIPE_SPEED 3
...
void flpDrawBird(uint16_t color) {
  ...
  if (color != CYBER_BG) {
    // Eye
    tft.fillRect(FLP_BIRD_X + 8, by + 2, 2, 2, CYBER_BG);
    // Beak
    tft.fillRect(FLP_BIRD_X + FLP_BIRD_SIZE, by + 4, 5, 4, AQ_BAR_ORANGE);
  } else {
    tft.fillRect(FLP_BIRD_X + FLP_BIRD_SIZE, by + 4, 5, 4, CYBER_BG);
  }
}
...
void flpSpawnPipe(int idx) {
  ...
  flpPipes[idx].gapY = random(FLP_GAP/2 + 16, FLP_AREA_H - FLP_GAP/2 - 16);
...
// ============================================================
// Layout: Left 130px = game, Right 30px = score panel
#define PNG_AREA_W 260
#define PNG_AREA_H 240
#define PNG_PANEL_X 264
#define PNG_PAD_W 6
#define PNG_PAD_H 41       // 22*1.875≈41
#define PNG_BALL_SIZE 6
#define PNG_P1_X 8         // Player paddle X (left side)
...
void pngDrawCenterLine() {
  for (int y = 0; y < PNG_AREA_H; y += 8) {
    tft.drawFastVLine(PNG_AREA_W / 2, y, 6, CYBER_DARK);
  }
}
...
void pngResetBall(int dir) {
  ...
  pngBallDX = dir * 3.5f;
  pngBallDY = (random(2) == 0) ? 2.0f : -2.0f;
}
...
void updatePong(int enc, bool encP, bool back) {
  ...
  if (enc != 0) {
    ...
    pngP1Y = constrain(pngP1Y + enc * 10, 0, PNG_AREA_H - PNG_PAD_H);
    pngDrawPaddle(PNG_P1_X, pngP1Y, CYBER_ACCENT);
  }
  ...
  if (abs(aiCenter - ballCenter) > 3) {
    ...
    if (aiCenter < ballCenter) pngAIY += 3;
    else pngAIY -= 2;
...
#define G48_GRID 4
#define G48_CELL 54       // 4*54=216px
#define G48_PAD 2
#define G48_X 4           // Grid start X
#define G48_Y 12          // Grid start Y
#define G48_PANEL_X 228
...
void g48DrawPanel() {
  tft.fillRect(G48_PANEL_X, 0, 92, 240, CYBER_BG);
  tft.setTextSize(1); tft.setTextColor(CYBER_LIGHT, CYBER_BG);
...
void drawHomeScreen() {
  ...
  if (!homeLoaded || homeRoomCount == 0) {
    tft.setTextSize(2);tft.setTextColor(CYBER_LIGHT); tft.setCursor(16, 8); tft.print("AKILLI EV");
    tft.setTextSize(1);tft.setTextColor(CYBER_DARK); tft.setCursor(240, 14); tft.print("K0:geri");
    tft.setTextSize(2);tft.setTextColor(ST77XX_WHITE, CYBER_BG);
    tft.setCursor(20, 70); tft.print("Cihaz listesi");
    tft.setCursor(20, 95); tft.print("bekleniyor...");
    tft.setTextSize(1);tft.setTextColor(CYBER_DARK);
    tft.setCursor(20, 140); tft.print("HA otomasyonu gerekli.");
    tft.setCursor(20, 165); tft.print("Bas: yenile");
    return;
  }
  
  if (homeMenuLevel == 0) {
    // Room list
    tft.setTextSize(2);tft.setTextColor(CYBER_LIGHT); tft.setCursor(16, 8); tft.print("ODALAR");
    tft.setTextSize(1);tft.setTextColor(CYBER_DARK); tft.setCursor(240, 14); tft.print("K0:geri");
    drawAlarmIcon();
    tft.setTextSize(2);
    for (int i = 0; i < homeRoomCount; i++) {
      int y = 40 + i * 28;
      if (i == homeRoomIdx) { tft.fillRoundRect(8, y-2, 304, 24, 4, CYBER_ACCENT); tft.setTextColor(CYBER_BG); }
      else { tft.fillRoundRect(8, y-2, 304, 24, 4, 0x18E3); tft.setTextColor(ST77XX_WHITE); }
      tft.setCursor(20, y+2); tft.print(homeRooms[i].name);
      char cnt[6]; sprintf(cnt, "(%d)", homeRooms[i].deviceCount);
      tft.setCursor(260, y+2); tft.print(cnt);
    }
  }
  else if (homeMenuLevel == 1) {
    // Device list for selected room
    HomeRoom &room = homeRooms[homeRoomIdx];
    tft.setTextSize(2);tft.setTextColor(CYBER_LIGHT); tft.setCursor(16, 8); tft.print(room.name);
    tft.setTextSize(1);tft.setTextColor(CYBER_DARK); tft.setCursor(240, 14); tft.print("K0:geri");
    drawAlarmIcon();
    tft.setTextSize(2);
    for (int i = 0; i < room.deviceCount; i++) {
      ...
      if (i == homeDevIdx) { tft.fillRoundRect(8, y-2, 304, 26, 4, CYBER_ACCENT); tft.setTextColor(CYBER_BG); }
      else { tft.fillRoundRect(8, y-2, 304, 26, 4, 0x18E3); tft.setTextColor(ST77XX_WHITE); }
      tft.setCursor(20, y+2); tft.print(dev.name);
      uint16_t sc = (strcmp(dev.state, "on") == 0) ? CYBER_GREEN : ST77XX_RED;
      if (i == homeDevIdx) sc = CYBER_BG;
      tft.setTextColor(sc);
      tft.setCursor(260, y+2);
      if (strcmp(dev.state, "on") == 0) tft.print("ON");
      else tft.print("OFF");
    }
    tft.setTextSize(1);tft.setTextColor(CYBER_DARK); tft.setCursor(8, 225);
    ...
    tft.setTextSize(2);tft.setTextColor(CYBER_LIGHT); tft.setCursor(16, 8); tft.print(dev.name);
    tft.setTextSize(1);tft.setTextColor(CYBER_DARK); tft.setCursor(240, 14); tft.print("K0:geri");
    
    // On/Off status
    tft.setTextSize(4);
    if (strcmp(dev.state, "on") == 0) { tft.setTextColor(CYBER_GREEN, CYBER_BG); tft.setCursor(60, 50); tft.print("ACIK"); }
    else { tft.setTextColor(ST77XX_RED, CYBER_BG); tft.setCursor(60, 50); tft.print("KAPALI"); }
    
    // Brightness bar (for lights)
    if (strcmp(dev.type, "light") == 0) {
      tft.setTextSize(2); tft.setTextColor(CYBER_DARK);
      tft.setCursor(16, 110); tft.print("Parlaklik:");
      int barW = map(dev.brightness, 0, 255, 0, 260);
      tft.fillRect(16, 135, 280, 16, CYBER_DARK);
      tft.fillRect(16, 135, barW, 16, CYBER_LIGHT);
      char pct[8]; sprintf(pct, "%d%%", map(dev.brightness, 0, 255, 0, 100));
      tft.setTextColor(ST77XX_WHITE, CYBER_BG);
      tft.setCursor(120, 160); tft.print(pct);
    }
    
    tft.setTextSize(1); tft.setTextColor(CYBER_DARK);
    tft.setCursor(8, 200); tft.print("Bas:ac/kapa");
    if (strcmp(dev.type, "light") == 0) {
      tft.setCursor(8, 215); tft.print("Cvr:parlaklik K0:geri");
    }
  }
}
...
void drawAlarmIcon() {
  int x = 308;
  tft.fillRect(x-14, 0, 18, 18, CYBER_BG);
...
void updateMelody() {
  ...
  unsigned long now = millis();
  unsigned long noteDur = (unsigned long)melodyDurs[melodyIdx];
  // Play note for 90% of duration, silent for 10% (staccato effect)
  unsigned long playTime = noteDur * 9 / 10;
  if (now - melodyLastMs >= playTime && buzzAttached && melodyNotes[melodyIdx] > 0) {
    vToneStop();  // brief silence before next note
  }
  if (now - melodyLastMs >= noteDur) {
    melodyIdx++;
...
void updateBacklight() {
  // Don't override dim/sleep brightness
  if (screenDimmed || screenSleeping) return;
  analogWrite(BL_PIN, backlightLevel);
}
...
void drawBrightnessScreen(bool full);
...
void enterDim() {
  ...
  screenDimmed = true;
  dimStartMs = millis();
  analogWrite(BL_PIN, constrain(backlightLevel * 1 / 100, 1, 3));  // ~1% brightness
...
void exitDim() {
  ...
  screenDimmed = false;
  analogWrite(BL_PIN, backlightLevel);  // Restore full brightness
}
...
void enterSleep() {
  ...
  // Restore landscape if in Tetris portrait mode
  ...
  Serial.println("Sleep mode ON — backlight OFF");
}
...
void wakeUp() {
  ...
  switch (currentMode) {
    ...
    case MODE_BRIGHT: drawBrightnessScreen(true); break;
...
void publishBacklightState() {
  if (!mqttClient.connected()) return;
  char buf[8]; sprintf(buf, "%d", backlightLevel);
  mqttClient.publish(TOPIC_BL_STATE, buf, true);
}

void drawBrightnessScreen(bool full) {
  if (full) { tft.fillScreen(CYBER_BG); tft.setTextSize(2); tft.setCursor(16,10);
    tft.setTextColor(CYBER_LIGHT); tft.print("PARLAKLIK"); drawAlarmIcon(); }
  
  // Brightness percentage
  int pct = map(backlightLevel, 0, 255, 0, 100);
  tft.fillRect(20, 55, 280, 45, CYBER_BG);
  tft.setTextSize(5); tft.setTextColor(CYBER_ACCENT, CYBER_BG);
  char buf[8]; sprintf(buf, "%d%%", pct);
  int16_t bx,by; uint16_t bw,bh;
  tft.getTextBounds(buf,0,0,&bx,&by,&bw,&bh);
  tft.setCursor((320-bw)/2, 60); tft.print(buf);
  
  // Bar
  tft.fillRect(30, 120, 260, 20, CYBER_DARK);
  int barW = map(backlightLevel, 0, 255, 0, 260);
  tft.fillRect(30, 120, barW, 20, CYBER_ACCENT);
  
  // Min/Max labels
  tft.setTextSize(1); tft.setTextColor(CYBER_DARK);
  tft.setCursor(30, 148); tft.print("0%");
  tft.setCursor(265, 148); tft.print("100%");
  
  tft.fillRect(0, 190, 320, 50, CYBER_BG);
  tft.setTextSize(2); tft.setTextColor(CYBER_DARK); tft.setCursor(16, 195);
  tft.print("Cevir: ayarla");
  tft.setTextSize(1); tft.setCursor(16, 220); tft.print("K0: geri");
}
...
void drawSettingsScreen(bool full) {
  if (full) {
    tft.fillScreen(CYBER_BG); tft.setTextSize(2); tft.setCursor(16,10);
    tft.setTextColor(CYBER_LIGHT); tft.print("AYARLAR"); drawAlarmIcon();
    sleepOptionIndex = getSleepOptionIndex();
  }

  const char* labels[] = {"Ses:", "Sicaklik:", "Nem:", "Oto Uyku:", "Alarm Sesi:"};
  char values[5][20];
  sprintf(values[0], "%d%%", volumeLevel);
  if (tempOffset >= 0) sprintf(values[1], "+%.1fC", tempOffset);
  else sprintf(values[1], "%.1fC", tempOffset);
  if (humOffset >= 0) sprintf(values[2], "+%.1f%%", humOffset);
  else sprintf(values[2], "%.1f%%", humOffset);
  sprintf(values[3], "%s", sleepLabels[sleepOptionIndex]);
  sprintf(values[4], "%s", rtttlNames[selectedAlarm]);

  for (int i = 0; i < SETTINGS_COUNT; i++) {
    int y = 38 + i * 37;
    tft.fillRect(8, y-2, 304, 34, CYBER_BG);
    tft.setTextSize(1);
    tft.setTextColor(CYBER_DARK);
    tft.setCursor(16, y); tft.print(labels[i]);
    tft.setTextSize(2);
    if (i == settingsField) {
      tft.setTextColor(CYBER_LIGHT, CYBER_BG);
    } else {
    ...
    tft.setCursor(16, y + 14); tft.print(values[i]);
    if (i == settingsField) {
      tft.setTextColor(CYBER_ACCENT);
      tft.setCursor(295, y + 14); tft.print("<");
    }
  }

  tft.fillRect(0, 225, 320, 15, CYBER_BG);
  tft.setTextSize(1); tft.setTextColor(CYBER_DARK);
  tft.setCursor(8, 228);
...
void connectWiFiAndSyncTime() {
  ...
  tft.setCursor(20,60); tft.print("Connecting WiFi...");
  ...
  if(WiFi.status()==WL_CONNECTED){
    tft.fillScreen(CYBER_BG); tft.setCursor(20,60); tft.setTextColor(CYBER_GREEN);
    tft.print("WiFi OK: "); tft.print(WiFi.localIP());
    configTime(gmtOffset_sec,daylightOffset_sec,ntpServer);
    tft.setCursor(20,85); tft.print("Time synced (GMT+3)"); delay(500);
    ...
    tft.setCursor(20,110); tft.print("MQTT connecting...");
    connectMqtt();
    tft.setCursor(20,135); if(mqttClient.connected()){tft.setTextColor(CYBER_GREEN);tft.print("MQTT OK!");}
    else{tft.setTextColor(AQ_BAR_ORANGE);tft.print("MQTT retry later...");}
    delay(800);
  } else {
    tft.fillScreen(CYBER_BG); tft.setCursor(20,85); tft.setTextColor(ST77XX_RED);
...
void mqttCallback(char* topic, byte* payload, unsigned int length) {
  ...
  // Backlight from HA (0-255)
  if(tp==TOPIC_BL_SET){
    int v=msg.toInt();
    backlightLevel=constrain(v,5,255);
    analogWrite(BL_PIN,backlightLevel);
    publishBacklightState();  // Backlight state
    if(currentMode==MODE_BRIGHT) drawBrightnessScreen(false);
    saveSettings();
  }
  ...
  if(tp==TOPIC_WEATHER){
    JsonDocument wdoc;
    if(!deserializeJson(wdoc, msg)){
      weatherTemp = wdoc["temp"] | 0.0f;
      weatherHum = wdoc["hum"] | 0;
      weatherWind = wdoc["wind"] | 0.0f;
      const char* c = wdoc["cond"] | "";
      strlcpy(weatherCond, c, sizeof(weatherCond));
      weatherValid = true;
      if(currentMode==MODE_CLOCK) drawWeather();
    }
  }
}
...
void publishMqttDiscovery() {
  ...
  ent[]={
    {"sensor","cyberclock_temp","Cyber Clock Temperature",TOPIC_TEMP,NULL,"°C","temperature"},
    {"sensor","cyberclock_hum","Cyber Clock Humidity",TOPIC_HUM,NULL,"%","humidity"},
    {"sensor","cyberclock_tvoc","Cyber Clock TVOC",TOPIC_TVOC,NULL,"ppb",NULL},
    {"sensor","cyberclock_co2","Cyber Clock CO2",TOPIC_CO2,NULL,"ppm","carbon_dioxide"},
    {"switch","cyberclock_alarm","Cyber Clock Alarm",TOPIC_ALARM_STATE,TOPIC_ALARM_SET,NULL,NULL},
    {"button","cyberclock_buzzer","Cyber Clock Buzzer",NULL,TOPIC_BUZZER_SET,NULL,NULL},
  };
  for(int i=0;i<6;i++){
    JsonDocument doc; doc["name"]=ent[i].n; doc["uniq_id"]=ent[i].id;
    doc["dev"]["ids"][0]="cyberclock_01"; doc["dev"]["name"]="Cyber Clock";
    doc["dev"]["mf"]="DIY"; doc["dev"]["mdl"]="Smart Cyber Clock v10.1-ST7789";
  ...
  // ===== Backlight brightness — number entity (5-255) =====
  {JsonDocument doc; doc["name"]="Cyber Clock Parlaklik"; doc["cmd_t"]=TOPIC_BL_SET;
   doc["stat_t"]=TOPIC_BL_STATE; doc["min"]=5; doc["max"]=255;
   doc["step"]=5; doc["icon"]="mdi:brightness-6";
   doc["uniq_id"]="cyberclock_backlight"; doc["dev"]["ids"][0]="cyberclock_01";
   doc["dev"]["name"]="Cyber Clock"; char b[512]; serializeJson(doc,b);
   mqttClient.publish("homeassistant/number/cyberclock_backlight/config",b,true);}

  // Remove old Backlight entities
  mqttClient.publish("homeassistant/switch/cyberclock_led_switch/config", "", true);
  mqttClient.publish("homeassistant/number/cyberclock_led_dimmer/config", "", true);
  mqttClient.publish("homeassistant/select/cyberclock_ledmode/config", "", true);

  // Volume number
  {JsonDocument doc; doc["name"]="Cyber Clock Volume"; doc["cmd_t"]=TOPIC_VOL_SET;
   doc["stat_t"]=TOPIC_VOL_STATE; doc["min"]=0; doc["max"]=100;
   doc["unit_of_meas"]="%"; doc["icon"]="mdi:volume-high";
   doc["uniq_id"]="cyberclock_vol"; doc["dev"]["ids"][0]="cyberclock_01";
   doc["dev"]["name"]="Cyber Clock"; char b[512]; serializeJson(doc,b);
   mqttClient.publish("homeassistant/number/cyberclock_vol/config",b,true);}

  // Temp calibration number
  {JsonDocument doc; doc["name"]="Cyber Clock Temp Offset"; doc["cmd_t"]=TOPIC_TCAL_SET;
   doc["stat_t"]=TOPIC_TCAL_STATE; doc["min"]=-15; doc["max"]=15;
   doc["step"]=0.5; doc["unit_of_meas"]="°C"; doc["icon"]="mdi:thermometer-alert";
   doc["uniq_id"]="cyberclock_tcal"; doc["dev"]["ids"][0]="cyberclock_01";
   doc["dev"]["name"]="Cyber Clock"; char b[512]; serializeJson(doc,b);
   mqttClient.publish("homeassistant/number/cyberclock_tcal/config",b,true);}

  // Hum calibration number
  {JsonDocument doc; doc["name"]="Cyber Clock Hum Offset"; doc["cmd_t"]=TOPIC_HCAL_SET;
   doc["stat_t"]=TOPIC_HCAL_STATE; doc["min"]=-20; doc["max"]=20;
   doc["step"]=1; doc["unit_of_meas"]="%"; doc["icon"]="mdi:water-percent";
   doc["uniq_id"]="cyberclock_hcal"; doc["dev"]["ids"][0]="cyberclock_01";
   doc["dev"]["name"]="Cyber Clock"; char b[512]; serializeJson(doc,b);
   mqttClient.publish("homeassistant/number/cyberclock_hcal/config",b,true);}

  // Sleep mode switch
  {JsonDocument doc; doc["name"]="Cyber Clock Uyku"; doc["cmd_t"]=TOPIC_SLEEP_SET;
   doc["stat_t"]=TOPIC_SLEEP_STATE; doc["icon"]="mdi:sleep";
   doc["uniq_id"]="cyberclock_sleep"; doc["dev"]["ids"][0]="cyberclock_01";
   doc["dev"]["name"]="Cyber Clock"; char b[512]; serializeJson(doc,b);
   mqttClient.publish("homeassistant/switch/cyberclock_sleep/config",b,true);}

  // Auto-sleep enable/disable switch
  {JsonDocument doc; doc["name"]="Cyber Clock Oto Uyku"; doc["cmd_t"]=TOPIC_AUTOSLEEP_SET;
   doc["stat_t"]=TOPIC_AUTOSLEEP_STATE; doc["icon"]="mdi:sleep-off";
   doc["uniq_id"]="cyberclock_autosleep"; doc["dev"]["ids"][0]="cyberclock_01";
   doc["dev"]["name"]="Cyber Clock"; char b[512]; serializeJson(doc,b);
...
void connectMqtt() {
  ...
  if(mqttClient.connect(mqtt_client_id,mqtt_user,mqtt_password,TOPIC_STATUS,0,true,"offline")){
    ...
    mqttClient.subscribe(TOPIC_BL_SET);        // Backlight
...
void updateEnvSensors(bool force) {
  ...
  // ENS160: measure and read
  ens160.set_envdata(curTemp, curHum);
  ensLastStatus = ens160.measure();
  // Read values regardless of status (some library versions return 1 even with valid data)
  uint16_t nT=ens160.getTVOC();
  uint16_t nC=ens160.geteCO2();
  if(nT!=0xFFFF && nT!=0 && nT<20000) curTVOC=nT;
  if(nC!=0xFFFF && nC!=0 && nC>=400 && nC<20000) curECO2=nC;
  // If stuck in status!=0, periodically re-init ENS160
  if(ensLastStatus != 0) {
    static unsigned long lastEnsRetry = 0;
    if(millis() - lastEnsRetry > 15000) {
      lastEnsRetry = millis();
      Wire.end(); delay(50); Wire.begin(SDA_PIN,SCL_PIN); delay(50);
      ens160.begin();
      delay(50);
      ens160.setMode(ENS160_OPMODE_STD);
      delay(50);
      // Re-init AHT too since we reset Wire
      aht.begin();
    }
  }
}
...
// Clock UI
// ============================================================
// ====== Clock UI Layout (320x240) ======
// Saat (y=15), Nem+Sıcaklık (y=78), Tarih (y=140), Hava durumu (y=185)
const int GRID_L=8,GRID_R=312,GRID_TOP=78,GRID_MID=130;
const int GRID_MID_X=(GRID_L+GRID_R)/2;
const int TOP_LABEL_Y=GRID_TOP+4,TOP_VALUE_Y=GRID_TOP+20;
const int DATE_Y=142;
const int WEATHER_Y=185;
...
const char* turkGun[] = {"Pazar","Pazartesi","Sali","Carsamba","Persembe","Cuma","Cumartesi"};
const char* turkAy[] = {"","Ocak","Subat","Mart","Nisan","Mayis","Haziran","Temmuz","Agustos","Eylul","Ekim","Kasim","Aralik"};

void drawDate() {
  time_t now; time(&now);
  struct tm ti; localtime_r(&now, &ti);
  char buf[40];
  sprintf(buf, "%d %s %d %s", ti.tm_mday, turkAy[ti.tm_mon+1], ti.tm_year+1900, turkGun[ti.tm_wday]);
  tft.fillRect(GRID_L, DATE_Y-2, GRID_R-GRID_L, 28, CYBER_BG);
  printCenteredText(String(buf), GRID_L, GRID_R, DATE_Y, CLR_DATE, CYBER_BG, 2);
}

// Translate HA weather condition to Turkish
const char* translateWeather(const char* cond) {
  if (strcmp(cond, "sunny") == 0) return "Gunesli";
  if (strcmp(cond, "clear-night") == 0) return "Acik";
  if (strcmp(cond, "partlycloudy") == 0) return "Parcali Bulutlu";
  if (strcmp(cond, "cloudy") == 0) return "Bulutlu";
  if (strcmp(cond, "rainy") == 0) return "Yagmurlu";
  if (strcmp(cond, "pouring") == 0) return "Saganak";
  if (strcmp(cond, "snowy") == 0) return "Karli";
  if (strcmp(cond, "snowy-rainy") == 0) return "Karla Karisik";
  if (strcmp(cond, "fog") == 0) return "Sisli";
  if (strcmp(cond, "hail") == 0) return "Dolu";
  if (strcmp(cond, "lightning") == 0) return "Firtina";
  if (strcmp(cond, "lightning-rainy") == 0) return "Gokgurultusu";
  if (strcmp(cond, "windy") == 0) return "Ruzgarli";
  if (strcmp(cond, "windy-variant") == 0) return "Ruzgarli";
  if (strcmp(cond, "exceptional") == 0) return "Olagandisi";
  return cond;
}

void drawWeather() {
  tft.fillRect(GRID_L, WEATHER_Y-2, GRID_R-GRID_L, 55, CYBER_BG);
  if (!weatherValid) {
    printCenteredText("Hava durumu bekleniyor...", GRID_L, GRID_R, WEATHER_Y+8, CYBER_GREY, CYBER_BG, 1);
    return;
  }
  char buf[64];
  const char* cond = translateWeather(weatherCond);
  int condLen = strlen(cond);
  // Short conditions (<=10 chars): single line, textSize 3
  // Long conditions: two rows, both textSize 2
  if (condLen <= 10) {
    sprintf(buf, "%s  %.1f'C", cond, weatherTemp);
    printCenteredText(String(buf), GRID_L, GRID_R, WEATHER_Y, CLR_WEATHER, CYBER_BG, 3);
  } else {
    // Condition + temp on same concept, two rows textSize 2
    sprintf(buf, "%s  %.1f'C", cond, weatherTemp);
    printCenteredText(String(buf), GRID_L, GRID_R, WEATHER_Y+2, CLR_WEATHER, CYBER_BG, 2);
  }
  // Humidity + Wind (smaller, below) — positioned based on layout
  int detailY = (condLen <= 10) ? WEATHER_Y+28 : WEATHER_Y+22;
  sprintf(buf, "Nem: %d%%   Ruzgar: %.1f km/h", weatherHum, weatherWind);
  printCenteredText(String(buf), GRID_L, GRID_R, detailY, CLR_WDETAIL, CYBER_BG, 1);
}

void initClockStaticUI() {
  tft.fillScreen(CYBER_BG);

  // MQTT indicator (top right, subtle)
  if(mqttClient.connected()){
    tft.setTextSize(1);tft.setTextColor(CYBER_GREEN);
    tft.setCursor(280,4);tft.print("MQTT");
  }

  // Grid lines (soft grey)
  tft.drawFastHLine(GRID_L,GRID_TOP,GRID_R-GRID_L,CYBER_GREY);
  tft.drawFastHLine(GRID_L,GRID_MID,GRID_R-GRID_L,CYBER_GREY);
  tft.drawFastVLine(GRID_MID_X,GRID_TOP,GRID_MID-GRID_TOP,CYBER_GREY);

  // Labels (textSize 1, soft grey)
  printCenteredText("Nem",GRID_L,GRID_MID_X,TOP_LABEL_Y,CYBER_GREY,CYBER_BG,1);
  printCenteredText("Sicaklik",GRID_MID_X,GRID_R,TOP_LABEL_Y,CYBER_GREY,CYBER_BG,1);

  // Date separator
  tft.drawFastHLine(GRID_L, WEATHER_Y-8, GRID_R-GRID_L, CYBER_GREY);
...
void drawClockTime(String hS,String mS,String sS){
  ...
  int16_t x1,y1;uint16_t w,h; tft.setTextSize(6);tft.getTextBounds(c,0,0,&x1,&y1,&w,&h);
  tft.setTextColor(CLR_CLOCK,CYBER_BG);tft.setCursor((320-w)/2,15);tft.print(c);}

void drawCO2Value(uint16_t e,uint16_t c){} // ENS160 disabled — stub

void drawEnvDynamic(float t,float h,uint16_t tv,uint16_t e){
  char b[16];
  // Clear value areas before redraw (prevent text ghosting)
  int valH = 28;
  tft.fillRect(GRID_L+1, TOP_VALUE_Y-2, GRID_MID_X-GRID_L-2, valH, CYBER_BG);
  tft.fillRect(GRID_MID_X+1, TOP_VALUE_Y-2, GRID_R-GRID_MID_X-2, valH, CYBER_BG);

  sprintf(b,"%.0f%%",h);printCenteredText(String(b),GRID_L,GRID_MID_X,TOP_VALUE_Y,CLR_HUM,CYBER_BG,3);
  sprintf(b,"%.1fC",t);printCenteredText(String(b),GRID_MID_X,GRID_R,TOP_VALUE_Y,CLR_TEMP,CYBER_BG,3);

  // Update date every minute
  static int lastMin = -1;
  time_t now; time(&now);
  struct tm ti; localtime_r(&now, &ti);
  if(ti.tm_min != lastMin){ lastMin = ti.tm_min; drawDate(); }
}
...
void drawMenu() {
  ...
  tft.setTextSize(2);tft.setCursor(20,8); tft.print("MOD SEC");
  if(mqttClient.connected()){tft.setTextColor(CYBER_GREEN);tft.setCursor(220,10);tft.print("[MQTT]");}
  const char* items[MENU_ITEMS]={"Monitor","Pomodoro","Alarm","Oyunlar","Parlaklik","Ev","Ayarlar","Uyku"};
  tft.setTextSize(2);
  ...
  for(int vi=0; vi<MENU_VISIBLE && (menuScrollOffset+vi)<MENU_ITEMS; vi++){
    ...
    int y = 35 + vi * 28;
    if(i==menuIndex){tft.fillRoundRect(10,y-4,300,24,4,CYBER_ACCENT);tft.setTextColor(CYBER_BG);}
    else{tft.fillRoundRect(10,y-4,300,24,4,0x18E3);tft.setTextColor(ST77XX_WHITE);}
    tft.setCursor(20,y);tft.print(items[i]);
    if(i==3 || i==5){
  ...
  // Scroll indicators
  if(menuScrollOffset > 0){tft.setTextColor(CYBER_DARK);tft.setCursor(155,25);tft.print("^");}
  if(menuScrollOffset + MENU_VISIBLE < MENU_ITEMS){tft.setTextColor(CYBER_DARK);tft.setCursor(155,35+MENU_VISIBLE*28);tft.print("v");}
  drawAlarmIcon();
}
...
void drawGamesMenu() {
  tft.fillScreen(CYBER_BG); tft.setTextSize(1); tft.setTextColor(CYBER_LIGHT);
  tft.setTextSize(2);tft.setCursor(20,8); tft.print("OYUNLAR");
  tft.setTextColor(CYBER_DARK);tft.setTextSize(1); tft.setCursor(240,14); tft.print("K0:geri");
  ...
  for(int vi=0; vi<GAMES_VISIBLE && (gamesScrollOffset+vi)<GAMES_COUNT; vi++){
    ...
    int y = 40 + vi * 30;
    if(i==gamesMenuIndex){tft.fillRoundRect(10,y-4,300,26,4,CYBER_ACCENT);tft.setTextColor(CYBER_BG);}
    else{tft.fillRoundRect(10,y-4,300,26,4,0x18E3);tft.setTextColor(ST77XX_WHITE);}
    tft.setTextSize(2);tft.setCursor(20,y); tft.print(gameNames[i]);
  }
  if(gamesScrollOffset > 0){tft.setTextColor(CYBER_DARK);tft.setCursor(155,30);tft.print("^");}
  if(gamesScrollOffset + GAMES_VISIBLE < GAMES_COUNT){tft.setTextColor(CYBER_DARK);tft.setCursor(155,40+GAMES_VISIBLE*30);tft.print("v");}
  drawAlarmIcon();
}
...
void drawPomodoroScreen(bool full) {
  ...
  if (pomoState == POMO_SELECT) {
    tft.fillRect(0,40,320,200,CYBER_BG);

    tft.setTextSize(2);tft.setTextColor(CYBER_GREY);
    tft.setCursor(80,48); tft.print("Zamani ayarla");
    ...
    int x = (320-w)/2;
    tft.setCursor(x, 85);
    tft.setTextColor(pomoField==0 ? CYBER_ACCENT : ST77XX_WHITE, CYBER_BG);
    tft.print(mBuf);
    tft.setTextColor(ST77XX_WHITE, CYBER_BG); tft.print(":");
    tft.setTextColor(pomoField==1 ? CYBER_ACCENT : ST77XX_WHITE, CYBER_BG);
    tft.print(sBuf);

    tft.setTextSize(1);
    tft.setTextColor(CYBER_GREY);
    tft.setCursor(x + 5, 140); tft.print("dakika");
    tft.setCursor(x + w - 40, 140); tft.print("saniye");

    tft.setTextSize(1);
    tft.setTextColor(CYBER_GREEN);
    tft.setCursor(60, 170); tft.print("Cevir: deger degistir");
    tft.setCursor(60, 185); tft.print("Bas: dk <-> sn");
    tft.setCursor(60, 200); tft.print("Uzun bas: BASLAT");
    tft.setTextColor(CYBER_DARK);
    tft.setCursor(60, 220); tft.print("K0: geri");
  } else {
    ...
    tft.fillCircle(cx,cy,65,CYBER_BG);
    unsigned long rem=dMs-elapsed;
    int remMin=rem/60000UL, remSec=(rem/1000UL)%60;
    char buf[8]; sprintf(buf,"%02d:%02d",remMin,remSec);
    int16_t x1,y1; uint16_t w,h;
    tft.setTextSize(5);tft.getTextBounds(buf,0,0,&x1,&y1,&w,&h);
    tft.setCursor(cx-w/2,cy-16);tft.setTextColor(ST77XX_WHITE,CYBER_BG);tft.print(buf);

    // Status text
    tft.fillRect(0,222,320,18,CYBER_BG);
    tft.setTextSize(1);
    if(pomoState==POMO_PAUSED) {
      tft.setTextColor(AQ_BAR_YELLOW,CYBER_BG);
      printCenteredText("DURDU  |  Bas: devam  |  K0: iptal", 0, 320, 226, AQ_BAR_YELLOW, CYBER_BG, 1);
    } else if(pomoState==POMO_DONE) {
      printCenteredText("BITTI!  |  Bas: yeni  |  K0: menu", 0, 320, 226, CYBER_GREEN, CYBER_BG, 1);
    } else {
      printCenteredText("CALISIYOR  |  Bas: durdur", 0, 320, 226, CYBER_ACCENT, CYBER_BG, 1);
    }
  }
}
...
void drawAlarmScreen(bool full) {
  ...
  tft.setTextSize(6);String disp=String(hB)+":"+String(mB);
  int16_t x1,y1;uint16_t w,h;tft.getTextBounds(disp,0,0,&x1,&y1,&w,&h);
  int x=(320-w)/2;tft.setCursor(x,60);
  ...
  tft.setTextSize(2);tft.fillRect(40,140,240,36,CYBER_BG);
  tft.setCursor(60,148);tft.setTextColor(ST77XX_WHITE,CYBER_BG);tft.print("Alarm:");
...
void drawAlarmRingingScreen(){
  tft.fillScreen(ST77XX_RED);tft.setTextSize(4);tft.setTextColor(ST77XX_WHITE,ST77XX_RED);
  tft.setCursor(70,60);tft.print("ALARM!");
  tft.setTextSize(2);tft.setCursor(40,130);tft.print("Herhangi tusa basin");
}
...
void updateAlertStateAndLED(){
  ...
  if(currentAlertLevel==ALERT_ALARM){
    ...
    if(now-lastAlertLedMs>120){
      lastAlertLedMs=now;
      alertLedState=!alertLedState;  // Alert: buzzer only (no LED)
    }
  }
    ...
    if(!co2AlertTriggered){
      ...
      tft.setTextSize(3); tft.setTextColor(ST77XX_WHITE, AQ_BAR_RED);
      tft.setCursor(30,30); tft.print("CO2 ALARM!");
      tft.setTextSize(5); tft.setCursor(40,80);
      tft.printf("%dppm", curECO2);
      tft.setTextSize(2); tft.setCursor(20,150);
      tft.print("Hava kalitesi cok kotu!");
      tft.setCursor(30,180); tft.print("Pencereyi acin!");
      tft.setCursor(30,210); tft.print("Tusa basin");
    }
    ...
    if(now-lastCo2BlinkMs>350){
      ...
      alertLedState=co2BlinkOn;  // Alert: buzzer only
    }
  }
  else {
...
void setup() {
  ...
  pinMode(BL_PIN,OUTPUT); pinMode(BUZZ_PIN,OUTPUT);
  analogWrite(BL_PIN,0);
  Wire.begin(SDA_PIN,SCL_PIN);
  SPI.begin(TFT_SCLK,-1,TFT_MOSI,TFT_CS);
  tft.init(240, 320); tft.setRotation(3); tft.invertDisplay(false); tft.fillScreen(CYBER_BG);
  connectWiFiAndSyncTime();
  Wire.end(); delay(200); Wire.begin(SDA_PIN,SCL_PIN); delay(200);
  
  // AHT21 init
  if(!aht.begin()) Serial.println("AHT21 fail"); else Serial.println("AHT21 OK");
  
  // ENS160 init — needs careful sequence after WiFi
  delay(100);
  bool ensOk = ens160.begin();
  if(!ensOk) {
    Serial.println("ENS160 begin fail, retrying...");
    delay(500);
  ...
  if(ensOk) {
    ...
    ens160.setMode(ENS160_OPMODE_STD);
    delay(100);
    // Do a few dummy reads to flush stale data
    for(int i=0; i<3; i++) { ens160.measure(); delay(100); }
...
void loop() {
  ...
  if(!inGame || screenSleeping){
    if(millis()-lastWifiCheck>30000){lastWifiCheck=millis();
      if(WiFi.status()!=WL_CONNECTED){WiFi.begin(ssid,password);}}  // Non-blocking reconnect
  ...
  updateBacklight(); updateMelody(); vToneUpdate();
  ...
  if(anyActivity) {
    resetActivityTimer();
    if(screenDimmed) exitDim();  // Any activity restores brightness
  }
  ...
  // === AUTO-DIM & AUTO-SLEEP TIMER ===
  if(sleepTimeoutMs > 0 && currentMode != MODE_SLEEP && !screenSleeping){
    unsigned long idle = millis() - lastActivityMs;
    // Phase 1: Dim after timeout (works on ALL screens including games)
    if(idle > sleepTimeoutMs && !screenDimmed){
      enterDim();
    }
    // Phase 2: Full sleep 2 min after dimming
    if(screenDimmed && millis() - dimStartMs > DIM_TO_SLEEP_MS){
      enterSleep();
      return;
    }
  }
  ...
  switch(currentMode){
    case MODE_MENU:
      ...
      if(encPressed){
        switch(menuIndex){
          ...
          case 4:currentMode=MODE_BRIGHT;drawBrightnessScreen(true);break;
    ...
    case MODE_BRIGHT:
      if(encStep!=0){
        backlightLevel=constrain((int)backlightLevel+encStep*10,5,255);
        analogWrite(BL_PIN,backlightLevel);
        publishBacklightState();  // Backlight state
        drawBrightnessScreen(false);
      }
      if(k0Pressed){
        saveSettings();
      ...
      if(encStep!=0){
        switch(settingsField){
          case 0: volumeLevel=constrain((int)volumeLevel+encStep*5,0,100);
                  ...
                  break;
          case 1: tempOffset=constrain(tempOffset+encStep*0.5f,-15.0f,15.0f); break;
          case 2: humOffset=constrain(humOffset+encStep*1.0f,-20.0f,20.0f); break;
          case 3: sleepOptionIndex=constrain(sleepOptionIndex+encStep,0,SLEEP_OPTION_COUNT-1);
                  ...
                  break;
          case 4: selectedAlarm=constrain((int)selectedAlarm+encStep,0,ALARM_COUNT-1);
                  playAlarmByIndex(selectedAlarm,false);
                  break;
        }
